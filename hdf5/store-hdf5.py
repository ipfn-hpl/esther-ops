...
import argparse
# Store date in https://10-24-30-188.fumanchu.direct.quickconnect.to:5001/
...
def create_hdf5(args, filename=H5FILE):
    ...
    try:
        with h5py.File(
            ...
            cgroup.create_group("ct")
            cgroup.create_group("st")
            cgroup.create_group("dt")
            ...
            f.attrs["experiment_name"] = args.experiment_name
...
def update_red_hdf5(args, hd5filename="data_with_metadata"):
    ...
    with h5py.File(file_path, "a") as f:
        if "raw-data" not in f:
            print("✗ 'raw-data' groups not exist, skipping")
            return
