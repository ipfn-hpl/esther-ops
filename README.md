...
CS-Studio [App](https://cs-studio.sourceforge.net/docbook/ch11.html)
...
2. Check if Gas Switchboard is powered on

- Wait 3 min and check if all Epics IOCs are running

```bash
ssh rpi5-gas
>systemctl status esther-gas-ioc.service
>exit

ssh rpi4-vacuum
>systemctl status esther-vacuum-ioc.service
>exit

ssh rpi4-mfc
>systemctl status esther-mfc-ioc.service
>exit

ssh galatea
>systemctl status epics-python-caput.service
>exit
```
<!-- comment systemctl status esther-vacuum-ioc.service -->
