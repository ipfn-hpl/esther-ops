...
class kistlerCom:
    ...
    def __init__(self, serial_port="/dev/ttyKistler", baud_rate=115200, read_timeout=1):
        """
    ...
    def setRange(self, range=300):
        """ """
        command = f"CH1:CT0:18,{range:d}\r".encode(
            "utf-8"
...
class quantel:
    ...
    def __init__(self, serial_port="/dev/ttyS0", baud_rate=9600, read_timeout=2):
        """
...
def parse_args():
    ...
    parser.add_argument(
        "-r", "--host_rp", default="192.168.0.34", help="Red Pitay IP address"
    )
    parser.add_argument(
        "-a", "--laserArm", action="store_true", help="Arm Quantel Laser"
    )
    parser.add_argument(
        "-s", "--laserStandby", action="store_true", help="Set Quantel in Standby"
    )
    parser.add_argument(
        "-c", "--redpitayaConfig", action="store_true", help="Check Red Pitaya Config"
    )
    parser.add_argument(
        "-f", "--fire", action="store_true", help="Fire ESTHER Pulse and acquisition"
    )
    parser.add_argument(
        "-t", "--trigger", action="store_true", help="Trigger Quantel Laser Q-Switch"
    )
    parser.add_argument(
        "-k", "--kistlerReset", action="store_true", help="Reset Kistler"
    )
    parser.add_argument(
        "-g", "--kistlerRange", type=int, default=0, help="Set Kistler Range"
    )
    parser.add_argument(
        "-m", "--mfcEpics", action="store_true", help="Start Test Gases MFCs"
    )
...
def taskRPitayaStart(host="192.168.0.34"):
    ...
    time.sleep(0.5)
    rp_query = subprocess.run(
        [
            "/home/esther/.local/bin/rpsa_client",
            "--remote",
            "--mode=start",
            f"--hosts={host:s}",
        ]
...
def checkRPitayaConfig(host="192.168.0.34"):
    # list_files = subprocess.run(["ls", "-l"], stdout=subprocess.DEVNULL)
    # print("The exit code was: %d" % list_files.returncode)
    rp_query = subprocess.run(
        [
...
def firePulse(host="192.168.0.34"):
