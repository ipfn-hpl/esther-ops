CREATE OR REPLACE FUNCTION get_signed_items(p_report_id INT, p_subsystem_id INT,  p_limit INT DEFAULT 10)
RETURNS TABLE (
      item_id INT, 
      seq_order SMALLINT, 
      time_date TIMESTAMP,
...
BEGIN
    RETURN QUERY
      ...
      FROM (
        ...
        WHERE i.subsystem_id = p_subsystem_id
        -- AND i.role_id = p_role_id
        ORDER BY c.time_date ASC;
        -- WHERE c.report_id = p_report_id AND i.subsystem_id = p_subsystem_id
END;
$$;
